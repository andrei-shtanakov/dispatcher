#!/usr/bin/env bash
# Re-vendor contracts/steward-gate-catalog/v1 at a new producer commit.
#
...
#
# The vendored surface is a single file: steward's profiles/gate-catalog.yaml
# lands here as gate-catalog.yaml. The staging/verify/swap machinery is the
# same as scripts/revendor_steward_gate_verdicts.sh, from which this script
# is an adapted copy.
#
...
#
# Usage:
#   scripts/revendor_steward_gate_catalog.sh <NEW_PIN> [--from <git-repo>]
#
...
SRC_PATH="profiles/gate-catalog.yaml"
DST="$REPO_ROOT/contracts/steward-gate-catalog/v1"
STAGING="$DST.staging"
PREV="$DST.prev"
...
# deep ("profiles/gate-catalog.yaml"); it is not derived from it. A future
...
note: pinned copy (repo-boundaries vendoring, ADR-ECO-003). Do not edit here —
  re-vendor with scripts/revendor_steward_gate_catalog.sh, which derives every
  value in this directory from the one commit it is given. Procedure:
  docs/revendor-steward-gate-catalog.md. Nothing in shipped code may read
  ../steward at run time.
EOF
...
re-vendored $SRC_PATH at $NEW_PIN
  ...
  next:       uv run pytest tests/test_gate_catalog_vendor.py \\
