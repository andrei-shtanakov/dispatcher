#!/usr/bin/env bash
# Re-vendor contracts/steward-gate-verdicts/v1 at a new producer commit.
#
...
#
# Usage:
#   scripts/revendor_steward_gate_verdicts.sh <NEW_PIN> [--from <git-repo>]
#
...
# segments ("contracts/gate-verdicts/v1"); it is not derived from it. A
# future SRC_SUBDIR at a different depth must update this number to match.
# Left uncoupled on purpose (same trade-off as the github-checker script):
# a mismatch is fail-closed — either the file-set check below (exit 3) or
# `tar` itself (exit 2) catches it — so a computed depth would add a moving
# part for no extra guarantee.
...
python3 "$REPO_ROOT/scripts/vendor_manifest.py" \
  ...
  --contract steward-gate-verdicts ||
...
re-vendored $SRC_SUBDIR at $NEW_PIN
  ...
  next:       uv run pytest tests/test_gate_verdicts_vendor.py \\
