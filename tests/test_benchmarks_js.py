"""Runs the Benchmarks panel client JS under Node (tests/web/).

Same discipline as tests/test_product_proposals_js.py: the harness parses the
shipped index.html, runs its WHOLE <script> in a VM over the
dependency-free DOM (tests/web/dom.js) and drives the real
`renderBenchmarks` code (including its click-to-select-leaderboard handler)
— nothing is sliced or simulated.
...
HARNESS = WEB / "benchmarks_harness.js"
INDEX_HTML = (
