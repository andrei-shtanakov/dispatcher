...
const SCENARIOS = [
  ...
  },
  {
    ...
    expectZero: false,
    expectStdout: /\[FAIL\] \d+\. selftest: an ordinary assertion mismatch/,
    forbidStdout: /cases passed, no async errors/,
  },
  {
    scenario: 'casethrow',
    ...
    expectZero: false,
    expectStdout: /\[FAIL\] \d+\. selftest: a case that throws/,
    forbidStdout: /cases passed, no async errors/,
  },
  {
    scenario: 'hang',
  ...
  },
  {
    ...
    why: 'a duplicate id makes one case invisible to the required-set check',
    expectZero: false,
    expectStderr: /duplicate case id/,
    forbidStdout: /cases passed, no async errors/,
  },
  {
    scenario: 'empty',
    why: 'a suite with no cases asserts nothing and must not exit 0',
    expectZero: false,
    expectStderr: /case list is EMPTY/,
    forbidStdout: /cases passed, no async errors/,
  },
  {
    scenario: 'missing-required',
    why: 'deleting a required case must fail, not shrink the suite quietly',
    expectZero: false,
    expectStderr: /required case\(s\) missing: 49b/,
    forbidStdout: /cases passed, no async errors/,
  },
  {
    scenario: 'all-skipped',
    why: 'a run that executed nothing is not a passing run',
    expectZero: false,
    expectStderr: /was SKIPPED/,
    forbidStdout: /cases passed, no async errors/,
  },
  {
    scenario: 'crash',
