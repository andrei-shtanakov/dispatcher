"""Runs the Run-status row's client JS under Node (tests/web/).

Same discipline as tests/test_benchmarks_js.py: the harness parses the
shipped index.html, runs its WHOLE <script> in a VM over the
dependency-free DOM (tests/web/dom.js) and drives the real click handler —
...
HARNESS = WEB / "run_status_harness.js"
INDEX_HTML = (
