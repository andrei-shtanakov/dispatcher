...
- [x] Публичное токенное сопоставление слага в `plan-fields`: приёмка кросс-репного запроса перестаёт совпадать по префиксу — PR #270 @owner:github:andrei-shtanakov @id:slug-token-match @epic:eco.plan-fields
      Принятие inbox-issue #254 от devtools (ADR-ECO-006). Сегодня `inbox.is_accepted`
      у потребителя выводит приёмку **подстрокой** в сыром тексте пункта `TODO.md`,
      поэтому `benchmark-2` ложно «принят» пунктом про `benchmark-20`. Слабость
      документирована в докстроке самой функции, и там же сказано, что ужесточать её —
      дело пакета: приватное строгое правило у потребителя = ровно тот дивергенс,
      который снимает ADR-ECO-005 D9. Это не гипотеза: в devtools#125 такое правило
      завелось в `todo_context.py` (круг 4) и было снято в круге 5, потому что
      `make inbox` печатал «принят» там, где `todo-context --issues` отвечал «issue
      нет» — два ответа об одном выводимом факте из одного репо.
      Наша часть — **публичное** правило в пакете, сопоставляющее слаг как ТОКЕН с
      границами по грамматике id (ADR-ECO-005 PF-2B, `[a-z0-9][a-z0-9._-]`).
      Недостаёт именно правила, а не грамматики: `parser.ITEM_ID_RE` уже публичная
      модульная константа и её импортирует `dispatcher/core/inventory.py` — не хватает
      только строки в `plan_fields.__all__` и самого сопоставления поверх неё, чтобы
      потребители звали пакет вместо своей регулярки. Форма API — на нас.
      Признак «сделано» **нашей половины**: публичное сопоставление в `plan-fields`,
      тест на пару из issue (для пункта только с `benchmark-20` слаг `benchmark-2` не
      совпадает, для пункта с `benchmark-2` — совпадает) и запись в `CONSUMING.md`.
      Вторая половина признака из issue — bump пина у devtools, удаление
      `todo_context.slug_re` и совпадение ответов `make inbox` и `todo-context
      --issues` об одной паре (issue, пункт) — работа соседа. Вахту `@blocked_by`
...
- [ ] Формат/линт-гейт в контуре spec-runner: `ruff format --check` + `ruff check` до выхода задачи на ревью @owner:github:andrei-shtanakov @id:spec-runner-lint-hook @epic:eco.spec-toolchain
      ...
      фикс-коммит, то есть лишний круг приёмки на каждый случай.
      `spec-runner.config.yaml` трекается в этом репо, значит гейт — наша половина:
      хук/шаг контура гоняет `uv run ruff format --check .` и `uv run ruff check .`
      до выхода задачи на ревью. Признак «сделано»: задача spec-runner с
      неотформатированным кодом не доходит до integration-PR — гейт красный локально,
      а не находкой терминального ревью на PR.
