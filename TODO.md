...
- [ ] Публичное токенное сопоставление слага в `plan-fields`: приёмка кросс-репного запроса перестаёт совпадать по префиксу @owner:github:andrei-shtanakov @id:slug-token-match @epic:eco.plan-fields
      ...
      границами по грамматике id (ADR-ECO-005 PF-2B, `[a-z0-9][a-z0-9._-]`; сегодня
      живёт приватно как `parser.ITEM_ID_RE`), чтобы потребители звали пакет вместо
      своей регулярки. Форма API — на нас.
      Признак «сделано» (после bump'а пина у devtools): `is_accepted("benchmark-2", …)`
      ложен для пункта, содержащего только `benchmark-20`, и истинен для пункта с
      `benchmark-2`; `todo_context.slug_re` и локальные токен-правила у потребителей
      удалены и заменены вызовом пакета; `make inbox` и `todo-context --issues` дают
      одинаковый ответ о приёмке одной пары (issue, пункт) — сегодня это единственное
      место, где они могут разойтись.
