...
FRESHNESS = {"source-ref": "source_ref", "observed-at": "observed_at",
             "recheck-by": "recheck_by"}


def _prov(repo: str, path: str, line: int) -> dict[str, Any]:
    return {"repo": repo, "remote_origin": None, "commit": None,
            "path": path, "line": line}
...
def parse_todo(
    ...
    for lineno, line in enumerate(text.splitlines(), start=1):
        ...
        if not item_id:
            diagnostics.append(_diag("PF-ID-MISSING", "warning", None, None, None,
                                     f"actionable item in repo {repo} has no @id "
                                     f"(line {lineno})", prov))
            continue
        node_id = f"todo://{repo}/{item_id}"
        owner = raw.get("owner")
        node = {
            "node_id": node_id, "kind": "operational_item", "id": item_id,
            "repo": repo, "title": title, "declared_status": status,
            "owner_role": owner, "trigger": raw.get("trigger"),
            "freshness": _freshness(raw), "tombstone": status == "closed",
            "raw": dict(raw), "provenance": prov,
        }
        nodes.append(node)
        parsed.append((node, raw))
        if owner is None:
            diagnostics.append(_diag("PF-OWNER-MISSING", "warning", node_id, None,
                                     None, f"{node_id} has no @owner", prov))
        elif not ROLE_RE.match(owner):
            diagnostics.append(_diag("PF-OWNER-GRAMMAR", "warning", node_id, None,
                                     None, f"@owner '{owner}' is not a role-slug",
                                     prov))
    ...
    for n in nodes:
        ...
        if prev is not None:
            diagnostics.append(_diag(
                "PF-ID-DUPLICATE", "error", n["node_id"], None, None,
    ...
    doc = {
        ...
        "generated_at": generated_at, "nodes": nodes, "references": references,
...
def _resolve(node, blocked, repo, nodes, ids):
    ...
    ref = {"kind": "blocked_by", "source_node_id": src, "raw_ref": blocked,
           "resolved_target": None, "legacy_blocker_ref": None, "provenance": prov}
    ...
    if m:
        ...
        if target_repo == repo and target_id in ids:
            ...
            return ref, {"kind": "blocked_by", "source_node_id": src,
                         "target_node_id": blocked}, None
        return ref, None, _diag(
            "PF-ID-DANGLING", "warning", src, blocked, None,
    ...
    if lm:
        ...
        if len(matches) == 1:
            ...
            return ref, {"kind": "blocked_by", "source_node_id": src,
                         "target_node_id": target}, None
        return ref, None, _diag(
            "PF-LEGACY-AMBIGUOUS", "warning", src, None, None,
            f"legacy reference {blocked} matches "
            f"{'more than one' if matches else 'no'} item in repo {repo}; "
            f"migrate to an @id", prov)
    return ref, None, None


def _diag(code, severity, subject, related, rule, message, prov):
    return {"code": code, "severity": severity, "subject_uri": subject,
            "related_uri": related, "rule_id": rule, "message": message,
            "provenance": prov}
