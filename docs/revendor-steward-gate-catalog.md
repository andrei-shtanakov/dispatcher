# Runbook: re-vendor `steward-gate-catalog/v1`

Canonical procedure for moving dispatcher's vendored copy of steward's
`profiles/gate-catalog.yaml` to a new producer commit. The copy was first
created by this same script (inbox #125); there is no separate creation
document to consult.

## When this runs

**Manually, after a change to `profiles/gate-catalog.yaml` has been accepted
...
every PR (`tests/test_gate_catalog_vendor.py`, never skipped) and the
scheduled `drift-steward-gate-catalog` job in
...
The catalog's own stability policy shapes what a re-vendor may bring: gate
ids are never renamed or reused, deprecations arrive as tombstones, and any
composition change bumps the catalog's `version` field. A `version` this
consumer does not support makes `dispatcher.core.gate_catalog.load_catalog`
raise — that is a consumer-code change reviewed on its own, not a plain
re-vendor.
...
> The file in `contracts/steward-gate-catalog/v1/` is, byte for byte,
...
`tests/test_gate_catalog_vendor.py` — proves only that someone edited three
...
```bash
scripts/revendor_steward_gate_catalog.sh <NEW_PIN>
```

It fetches `NEW_PIN` from `https://github.com/andrei-shtanakov/steward` into
a throwaway bare object store, extracts `profiles/gate-catalog.yaml` into a
staging directory beside the vendored copy, verifies the staged file
against the commit's blob and the file set in both directions, writes
`PINNED.txt`, regenerates `manifest.json` from the same SHA (via
`scripts/vendor_manifest.py --contract steward-gate-catalog`), checks the
...
The script is an adapted copy of `revendor_steward_gate_verdicts.sh` — same
...
`contracts/steward-gate-catalog/v1/` missing with its contents parked in an
untracked `v1.prev/` sibling; the recovery is
`mv contracts/steward-gate-catalog/v1.prev contracts/steward-gate-catalog/v1`.

**Offline variant.** `--from <git-repo>` reads the commit out of a local
repository's object database instead:

```bash
scripts/revendor_steward_gate_catalog.sh <NEW_PIN> --from ../steward
```
...
`tests/test_gate_catalog_vendor.py` holds its own copy of the pin:
...
proves nothing. `tests/test_gate_catalog.py` additionally pins the catalog's
*content* the consumer stands on — the obligation vocabulary and the v1
composition (19 active + `GC-APPROVAL-MISSING`); a re-vendor that changes
the composition updates those assertions deliberately, together with any
consumer behaviour the change implies.

### 4. Run the gate

```bash
uv run pytest tests/test_gate_catalog_vendor.py tests/test_gate_catalog.py \
  tests/test_governance_collector.py -v
uv run pytest
```
...
| Contract | Procedure |
|---|---|
| `contracts/steward-gate-catalog/v1` | this runbook |
