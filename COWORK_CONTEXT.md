...
1. **Read-only с белым списком мутаций**: никогда не пишет в наблюдаемые проекты,
   кроме явного белого списка действий (sync: `pull`/`create-pr` + spec-runner
   config editor) — только PR, только по явному клику, только через
   `github-checker propose-pr` (dispatcher не пишет и не мержит); SQLite строго
