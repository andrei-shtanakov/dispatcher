...
Maestro, arbiter, spec-runner, proctor-a; список расширяемый). Читает
...
## API (публичный контракт, его же будут потреблять TUI и VSCode-плагин)
...
- **Stage 2**: TUI (textual), переиспользует `dispatcher.core` напрямую.
- **Stage 3**: VSCode-плагин поверх HTTP API.
