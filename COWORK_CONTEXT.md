...
## API (публичный контракт, его же потребляют VSCode-плагин и MCP-тулзы)

`GET /api/overview` · `/api/projects/{name}` ·
`/api/errors?limit&days&project&service` · `/api/models` · `/api/contracts` · `/api/projects/{name}/onboarding` ·
`POST /api/projects/{name}/spec-runner-config/suggest` · `POST /api/projects/{name}/spec-runner-config/suggest/cancel` · `GET /api/spec-runner-config/suggest-availability` (dashboard-only, не публичный контракт)

## Жёсткие инварианты

1. **Read-only с белым списком мутаций**: никогда не пишет в наблюдаемые проекты,
   кроме явного белого списка действий — только по явному клику, только PR,
   всё делегировано `github-checker` (dispatcher сам не пишет git и не мержит):
   sync-действия через `pull`/`open-pr`, spec-runner config editor через
   `propose-pr --edit --if-match` (контент — во временный worktree, живое
   дерево не трогается); SQLite строго `mode=ro` (`immutable=1` запрещён —
   источники живые WAL-базы).
